...
test("New Olympiad has its own student route and dashboard entry",()=>{const page=readFileSync("app/training/new-olympiad/page.tsx","utf8"),dashboard=readFileSync("app/dashboard/page.tsx","utf8");assert.match(page,/variant="generated"/);assert.match(dashboard,/Авторская тренировка/);assert.match(dashboard,/Официальный вариант «Взлёта» 2025\/2026/);assert.match(dashboard,/\/training\/new-olympiad/)});
