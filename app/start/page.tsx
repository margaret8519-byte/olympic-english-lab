...
export default function Start(){
  ...
  async function handleSubmit(event:FormEvent<HTMLFormElement>){
    ...
    try{const{joinedClass}=await syncStudentProfile(p,joinCode),saved=joinedClass?{...p,grade:String(joinedClass.grade??p.grade),classLetter:joinedClass.class_letter??p.classLetter}:p;localStorage.setItem("studentProfile",JSON.stringify(saved));localStorage.setItem("studentProfileSync","synced");if(joinedClass)localStorage.setItem("studentClass",JSON.stringify(joinedClass));router.push("/dashboard")}
    catch(error){if(joinCode){if(process.env.NODE_ENV==="development")console.error("[Class Join Error]",error);setJoinError(getClassJoinErrorMessage(error));setSaving(false);return}localStorage.setItem("studentProfile",JSON.stringify(p));localStorage.setItem("studentProfileSync","local");console.error("[Supabase profile sync error]",error);router.push("/dashboard")}
  }
