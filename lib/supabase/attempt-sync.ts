...
import { normalizeAnswer } from "../answer-checker.ts";
...
const isCorrect=(question:AdaptiveQuestion,answer:string)=>{const normalized=normalizeAnswer(answer);return question.acceptedAnswers.some(value=>normalizeAnswer(value)===normalized)};
...
export function aggregateSkillStats(allAnswers:{skill:string|null;subskill:string|null;is_correct:boolean|null}[],studentId:string){const grouped=new Map<string,{skill:string;subskill:string;attempts:number;correct:number;incorrect:number}>();for(const answer of allAnswers){if(answer.is_correct!==true&&answer.is_correct!==false)continue;const skill=answer.skill||"Unknown",subskill=answer.subskill||"Unknown",key=`${skill}\u0000${subskill}`,row=grouped.get(key)||{skill,subskill,attempts:0,correct:0,incorrect:0};row.attempts++;row.correct+=answer.is_correct?1:0;row.incorrect+=answer.is_correct?0:1;grouped.set(key,row)}return[...grouped.values()].map(row=>({student_id:studentId,...row,accuracy:row.attempts?Math.round(row.correct/row.attempts*100):0}))}

async function updateSkillStats(supabase:SupabaseClient,studentId:string){const{data:allAnswers,error:readError}=await supabase.from("attempt_answers").select("skill,subskill,is_correct").eq("student_id",studentId).not("is_correct","is",null);if(readError)throw readError;const payload=aggregateSkillStats(allAnswers||[],studentId);if(!payload.length)return;const{error}=await supabase.from("skill_stats").upsert(payload,{onConflict:"student_id,skill,subskill"});if(error)throw error}
