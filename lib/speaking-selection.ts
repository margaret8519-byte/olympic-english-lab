...
export function selectSpeakingVisitTask(tasks:SpeakingSet[],grade:number,shownTaskIds:string[]=[],random:()=>number=Math.random){const shown=new Set(shownTaskIds),remaining=speakingTasksForGrade(tasks,grade).filter(task=>!shown.has(task.id));if(!remaining.length)return null;const index=Math.min(remaining.length-1,Math.floor(Math.max(0,random())*remaining.length));return remaining[index]}
